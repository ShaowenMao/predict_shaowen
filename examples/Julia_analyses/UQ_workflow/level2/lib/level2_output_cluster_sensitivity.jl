...
function print_help()
    println("Usage:")
    println("  julia --project=examples/Julia_analyses/UQ_workflow examples/Julia_analyses/UQ_workflow/level2/lib/level2_output_cluster_sensitivity.jl [options]")
    println()
    println("Options:")
...
function main(args::Vector{String})
    ...
    for fraction in fractions
        ...
        for (widx, proxy) in enumerate(proxies)
            state = Level2StateObject.build_window_state(proxy["log_perms"],
                                                         proxy["raw_perms"],
                                                         proxy["window"],
                                                         proxy["source_path"],
                                                         proxy["source_label"],
