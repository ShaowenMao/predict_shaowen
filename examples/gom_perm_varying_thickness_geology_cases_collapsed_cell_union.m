function results = gom_perm_varying_thickness_geology_cases_collapsed_cell_union(outputDir, varargin)
% Generate PREDICT datasets using collapsed layers and cell_union_psmear.
%
% This driver is a separate, non-destructive variant of
% gom_perm_varying_thickness_geology_cases. It uses the same 6 thickness
% scenarios and 27 geology cases, but before building each FaultedSection it
% collapses adjacent equal-lithology layers within each window side. For
% example, SSSC becomes SC and SCCC becomes SC, with the new package
% thickness equal to the sum of the merged layers and the burial depth zmax
% equal to the thickness-weighted average of the original layers.
%
% Clay/sand material placement is fixed to SmearOverlapRule =
% 'cell_union_psmear'. The legacy driver is intentionally left untouched.
%
% Default experiment:
...
%   results = gom_perm_varying_thickness_geology_cases_collapsed_cell_union( ...
%       'D:\codex_gom\thickness_cases_collapsed_cell_union')
%
...
%   'SmearOverlapRule'       - must be 'cell_union_psmear'. This parameter is
%                              accepted only to keep output metadata explicit.
%
% Output structure:
%   outputDir/
...
if nargin < 1 || isempty(outputDir)
    outputDir = fullfile(pwd, ...
        'gom_varying_thickness_geology_cases_collapsed_cell_union');
end
...
parser.addParameter('SmearOverlapRule', 'cell_union_psmear', @(x) ischar(x) || isstring(x));
parser.parse(varargin{:});
opt = parser.Results;
assert(strcmpi(string(opt.SmearOverlapRule), "cell_union_psmear"), ...
       ['This collapsed-input driver is fixed to SmearOverlapRule = ' ...
        'cell_union_psmear. Use gom_perm_varying_thickness_geology_cases ' ...
        'for other overlap rules.'])
opt.SmearOverlapRule = 'cell_union_psmear';
opt.CollapseAdjacentLithology = true;
...
collapsedThicknessScenarioTable = collapseScenarioPatternTable(thicknessScenarioTable);
...
writetable(collapsedThicknessScenarioTable, ...
           fullfile(outputDir, 'collapsed_thickness_scenario_definitions.csv'));
save(fullfile(outputDir, ...
     'collapsed_cell_union_geology_case_definitions.mat'), ...
     'geologyCaseTable', 'thicknessScenarioTable', ...
     'collapsedThicknessScenarioTable', 'windows', 'opt', '-v7.3');

runMetaRows = {};
metadataCsv = fullfile(tableDir, ...
    'collapsed_cell_union_geology_run_metadata.csv');
...
for iscen = 1:numel(scenarioIds)
    ...
    for iw = 1:numel(windows)
        ...
        for icase = 1:height(geologyCaseTable)
            ...
            expected = struct( ...
                'Kind', 'VariedThicknessGeologyRunCollapsedCellUnion', ...
                'ScenarioIndex', scenarioId, ...
                'ScenarioLabel', scenarioLabel, ...
                'ScenarioName', scenarioName, ...
                'Window', window, ...
                'OriginalFWPattern', char(scenarioWindow.FWPattern), ...
                'OriginalHWPattern', char(scenarioWindow.HWPattern), ...
                'FWPattern', char(variedWindowOpt.collapsedPattern{1}), ...
                'HWPattern', char(variedWindowOpt.collapsedPattern{2}), ...
                'FWThickness', layerVectorString(variedWindowOpt.thick{1}), ...
                'HWThickness', layerVectorString(variedWindowOpt.thick{2}), ...
                'FWZmax', layerVectorString(variedWindowOpt.zmax{1}), ...
                'HWZmax', layerVectorString(variedWindowOpt.zmax{2}), ...
            ...
            runMetaRows(end+1, :) = {scenarioId, scenarioLabel, scenarioName, ...
                                     ...
                                     char(variedWindowOpt.collapsedPattern{1}), ...
                                     char(variedWindowOpt.collapsedPattern{2}), ...
                                     layerVectorString(variedWindowOpt.thick{1}), ...
                                     layerVectorString(variedWindowOpt.thick{2}), ...
                                     layerVectorString(variedWindowOpt.zmax{1}), ...
                                     layerVectorString(variedWindowOpt.zmax{2}), ...
                                     char(opt.SmearOverlapRule), ...
                                     logical(opt.CollapseAdjacentLithology), ...
            ...
            runMetaTable = cell2table(runMetaRows, 'VariableNames', ...
                {'ScenarioIndex', 'ScenarioLabel', 'ScenarioName', ...
                 'Window', 'OriginalFWPattern', 'OriginalHWPattern', ...
                 'CollapsedFWPattern', 'CollapsedHWPattern', ...
                 'CollapsedFWThickness', 'CollapsedHWThickness', ...
                 'CollapsedFWZmax', 'CollapsedHWZmax', ...
                 'SmearOverlapRule', 'CollapseAdjacentLithology', ...
                 'CaseIndex', 'CaseLabel', 'FaultingDepth', ...
                 'SandVcl', 'ClayVcl', 'TargetN', 'NumAttempts', 'NumRejected', ...
...
results.collapsedThicknessScenarios = collapsedThicknessScenarioTable;
results.geologyCases = geologyCaseTable;
results.runMetadata = runMetaTable;
results.options = opt;

save(fullfile(outputDir, ...
     'gom_varying_thickness_geology_cases_collapsed_cell_union_results.mat'), ...
     'results', '-v7.3');
end
...
function collapsedTable = collapseScenarioPatternTable(scenarioTable)
% Add collapsed S/C pattern columns for quick run auditing.

collapsedTable = scenarioTable;
collapsedTable.CollapsedFWPattern = strings(height(scenarioTable), 1);
collapsedTable.CollapsedHWPattern = strings(height(scenarioTable), 1);
for i = 1:height(scenarioTable)
    collapsedTable.CollapsedFWPattern(i) = string(collapsePatternString(scenarioTable.FWPattern(i)));
    collapsedTable.CollapsedHWPattern(i) = string(collapsePatternString(scenarioTable.HWPattern(i)));
end
end


function patternOut = collapsePatternString(patternIn)
% Collapse consecutive equal S/C symbols without changing order.

pattern = upper(char(patternIn));
assert(~isempty(pattern), 'S/C pattern cannot be empty.')
assert(all(pattern == 'S' | pattern == 'C'), ...
       'Pattern must contain only S and C values.')
keep = [true, diff(double(pattern)) ~= 0];
patternOut = pattern(keep);
end
...
% Apply one geology case and collapse adjacent equal-lithology packages.

variedWindowOpt = baseWindowOpt;
variedWindowOpt.zf = [caseInfo.FaultingDepth, caseInfo.FaultingDepth];
variedWindowOpt.originalPattern = {char(scenarioWindow.FWPattern), ...
                                   char(scenarioWindow.HWPattern)};
variedWindowOpt.vcl = { ...
    patternToVcl(variedWindowOpt.originalPattern{1}, caseInfo.SandVcl, caseInfo.ClayVcl), ...
    patternToVcl(variedWindowOpt.originalPattern{2}, caseInfo.SandVcl, caseInfo.ClayVcl)};
variedWindowOpt.collapsedPattern = variedWindowOpt.originalPattern;

for side = 1:2
    [variedWindowOpt.thick{side}, variedWindowOpt.vcl{side}, ...
     variedWindowOpt.zmax{side}, variedWindowOpt.collapsedPattern{side}] = ...
        collapseAdjacentLayers(variedWindowOpt.thick{side}, ...
                               variedWindowOpt.vcl{side}, ...
                               variedWindowOpt.zmax{side}, ...
                               variedWindowOpt.originalPattern{side});
end
end
...
function [thickOut, vclOut, zmaxOut, patternOut] = collapseAdjacentLayers(thick, vcl, zmax, pattern)
% Merge consecutive same-lithology layers into one thicker parent package.
...
assert(numel(pattern) == numel(thick), ...
       'S/C pattern length must match the base thickness vector.')
assert(numel(vcl) == numel(thick) && numel(zmax) == numel(thick), ...
       'Layer thickness, Vcl, and zmax vectors must have matching lengths.')

starts = [1, find(diff(double(pattern)) ~= 0) + 1, numel(pattern) + 1];
nGroups = numel(starts) - 1;
thickOut = zeros(1, nGroups);
vclOut = zeros(1, nGroups);
zmaxOut = zeros(1, nGroups);
...
% Thickness-weighted mean for collapsed layer burial depth.

value = sum(values(:) .* weights(:)) ./ sum(weights(:));
end


function row = asRow(x)
% Normalize scalar/vector layer data to a row vector.
...
% Store collapsed layer vectors compactly in metadata/checkpoints.

s = strjoin(compose('%.8g', asRow(x)), ' ');
end
...
tf = strcmpi(string(checkpointInfo.Kind), string(expected.Kind)) && ...
     ...
     strcmpi(string(checkpointInfo.OriginalFWPattern), string(expected.OriginalFWPattern)) && ...
     strcmpi(string(checkpointInfo.OriginalHWPattern), string(expected.OriginalHWPattern)) && ...
     ...
     strcmpi(string(checkpointInfo.FWThickness), string(expected.FWThickness)) && ...
     strcmpi(string(checkpointInfo.HWThickness), string(expected.HWThickness)) && ...
     strcmpi(string(checkpointInfo.FWZmax), string(expected.FWZmax)) && ...
     strcmpi(string(checkpointInfo.HWZmax), string(expected.HWZmax)) && ...
