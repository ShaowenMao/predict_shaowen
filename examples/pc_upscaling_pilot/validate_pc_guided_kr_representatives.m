...
% slices in the same case/window. The reduced result is the one dynamic Kr
% curve selected at the scalar effective-Swi medoid from Pc upscaling.
%
...
assert(exist(proxySummaryCsv, 'file') == 2, ...
    'Swi-medoid Kr summary not found: %s', proxySummaryCsv);
...
assert(all(ismember(required, proxyTable.Properties.VariableNames)), ...
    'Swi-medoid Kr summary lacks required validation columns.');
...
for g = 1:height(keys)
    ...
    assert(height(proxyRows) == 1, ...
        'Expected one Swi-medoid curve for case %d, window %s; found %d.', ...
...
validation = cell2table(rows, 'VariableNames', { ...
    ...
    'FullCurveCount', 'SwiMedoidSourceRow', 'SwiMedoidSliceIndex', ...
    'SwiMedoidEffectiveSwi', 'SwiMedoidBrineCoreyExponent', ...
    'SwiMedoidGasCoreyExponent', 'FullKrMedoidSourceRow', ...
    'FullKrMedoidSliceIndex', 'FullKrMedoidEffectiveSwi', ...
    'FullKrMedoidBrineCoreyExponent', 'FullKrMedoidGasCoreyExponent', ...
    'RmseToFullKrMedoid', 'MaxAbsDifferenceToFullKrMedoid', ...
    'SwiMedoidMeanDistanceToFullFamily', ...
    'KrMedoidMeanDistanceToFullFamily', ...
    'CentralityRatio', 'SwiMedoidCentralityRank', 'IsExactKrMedoidShape', ...
    'DistanceToNearestFullShape', 'NearestFullShapeSliceIndex', 'Accepted'});

outputCsv = fullfile(outputDir, 'swi_medoid_kr_validation.csv');
writetable(validation, outputCsv);
fprintf('Saved Swi-medoid Kr validation: %s\n', outputCsv);
...
disp(validation(:, {'Level3CaseId', 'Window', 'SwiMedoidSliceIndex', ...
    'FullKrMedoidSliceIndex', 'RmseToFullKrMedoid', ...
    'MaxAbsDifferenceToFullKrMedoid', 'Accepted'}));
end
