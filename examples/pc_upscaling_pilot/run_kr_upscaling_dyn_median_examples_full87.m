...
cfg.productionMethodConfigHash = lower(strtrim(string(envOrDefault( ...
    "KR_DYN_PRODUCTION_METHOD_CONFIG_SHA256", ""))));
...
if cfg.exportReservoirReady && isFullProduction
    ...
    fullSliceOutputs = export_reservoir_ready_pc_kr_cases( ...
        ...
        'ReplayManifestInput', cfg.replaySummaryCsv, ...
        'ProductionMethodConfigHash', cfg.productionMethodConfigHash);
...
% Verify that compiled AMGCL MEX functions and the AD wrapper are callable.
...
try
    ...
    mexOk = isfinite(relResidual) && relResidual < 1e-6;
    if ~mexOk
    ...
    adSolver = configureGenericAmgclSolver(AMGCLSolverAD( ...
        'maxIterations', 1000, 'tolerance', 1e-8));
    adSolver.setCoarsening('aggregation');
    adSolver.setRelaxation('spai0');
    adSolver.setSolver('bicgstab');
    [xAd, adReport] = adSolver.solveLinearSystem(A, b);
    adRelResidual = norm(A*xAd - b) ./ max(norm(b), realmin);
    report(end+1) = sprintf("AMGCLSolverAD smoke solve relative residual: %.3e", ...
        adRelResidual);
    if isfield(adReport, 'Iterations')
        report(end+1) = sprintf("AMGCLSolverAD iterations: %d", adReport.Iterations);
    end
    ok = isfinite(adRelResidual) && adRelResidual < 1e-6;
    if ~ok
        report(end+1) = "AMGCL check failed: AMGCLSolverAD residual is too large.";
    end
catch err
...
function solver = configureGenericAmgclSolver(solver)
% Normalize older MRST snapshots that omit the regular AMGCL strategy ID.

if ~isfield(solver.amgcl_setup, 'solver_id')
    solver.amgcl_setup.solver_id = 1;
end
end
...
marker = fullfile(outputRoot, '.codex_dyn_patch_v22');
...
patchTextFile(fullfile(outputRoot, 'upscaling', 'dynamic3Drun.m'), { ...
    "rock2.poro = nan(G2.cells.num, 1);" + newline + ...
    "rock2.poro(1:G.cells.num) = rock.poro(idG);" + newline + ...
    "rock2.poro(G.cells.num+1:end) = mean(rock.poro(~fault3D.Grid.isSmear));" + newline + ...
    "" + newline + ...
    "rock2.perm = nan(G2.cells.num, 6);" + newline + ...
    "rock2.perm(1:G.cells.num, :) = rock.perm(idG, :);" + newline + ...
    "rock2.perm(G.cells.num+1:end, :) = repmat(mean(rock.perm(~fault3D.Grid.isSmear, :)), ..." + newline + ...
    "                                                G.cartDims(1)*G.cartDims(2), 1);", ...
    "sandCells = ~fault3D.Grid.isSmear;" + newline + ...
    "if any(sandCells)" + newline + ...
    "    boundarySandPoro = mean(rock.poro(sandCells));" + newline + ...
    "    boundarySandPerm = mean(rock.perm(sandCells, :), 1);" + newline + ...
    "else" + newline + ...
    "    assert(isfield(fault3D.Grid, 'boundarySandPoro') && ..." + newline + ...
    "           isfield(fault3D.Grid, 'boundarySandPerm'), ..." + newline + ...
    "           'All-smear maps require stored parent-sand boundary properties.');" + newline + ...
    "    boundarySandPoro = fault3D.Grid.boundarySandPoro;" + newline + ...
    "    boundarySandPerm = fault3D.Grid.boundarySandPerm;" + newline + ...
    "    fprintf('    All-smear map: using stored parent-sand properties for the numerical boundary layer.\n');" + newline + ...
    "end" + newline + ...
    "assert(isfinite(boundarySandPoro) && boundarySandPoro > 0, ..." + newline + ...
    "       'Boundary-layer sand porosity must be positive and finite.');" + newline + ...
    "assert(all(isfinite(boundarySandPerm)) && all(boundarySandPerm([1,4,6]) > 0), ..." + newline + ...
    "       'Boundary-layer sand permeability must be positive and finite.');" + newline + ...
    "" + newline + ...
    "rock2.poro = nan(G2.cells.num, 1);" + newline + ...
    "rock2.poro(1:G.cells.num) = rock.poro(idG);" + newline + ...
    "rock2.poro(G.cells.num+1:end) = boundarySandPoro;" + newline + ...
    "" + newline + ...
    "rock2.perm = nan(G2.cells.num, 6);" + newline + ...
    "rock2.perm(1:G.cells.num, :) = rock.perm(idG, :);" + newline + ...
    "rock2.perm(G.cells.num+1:end, :) = repmat(boundarySandPerm, ..." + newline + ...
    "                                                G.cartDims(1)*G.cartDims(2), 1);" ...
    ; ...
...
patchTextFile(fullfile(outputRoot, 'upscaling', 'dynamicBLrun.m'), { ...
    ...
    "assert(isfinite(swc) && swc >= 0 && swc < 1, ..." + newline + ...
    "               'Pc-derived effective Swi must satisfy 0 <= Swi < 1.');" + newline + ...
    "        sourceSwcBounds = sort(fluid.krPts.og(:,2));" + newline + ...
    "        if swc < sourceSwcBounds(1) || swc > sourceSwcBounds(end)" + newline + ...
    "            fprintf(['                Pc-derived effective Swi %.8g lies outside ', ..." + newline + ...
    "                     'constituent endpoints [%.8g, %.8g]; retaining the exact ', ..." + newline + ...
    "                     'upscaled endpoint for 1D matching.\n'], ..." + newline + ...
    "                    swc, sourceSwcBounds(1), sourceSwcBounds(end));" + newline + ...
    "        end" ...
    ; ...
    "model.PVTPropertyFunctions = model.PVTPropertyFunctions.setStateFunction('PoreVolume', MyPvMult(model));", ...
    "model.PVTPropertyFunctions = model.PVTPropertyFunctions.setStateFunction('PoreVolume', BlackOilPoreVolume(model));"; ...
    ...
    "                    nls1D = getNonLinearSolver(model, 'TimestepStrategy', 'iteration', 'useCPR', false);" + newline + ...
    ...
    "                            if ~isfield(nls1D.LinearSolver.amgcl_setup, 'solver_id')" + newline + ...
    "                                nls1D.LinearSolver.amgcl_setup.solver_id = 1;" + newline + ...
    "                            end" + newline + ...
...
"", ...
"        swc = 1 - max(sg);", ...
"        assert(isfinite(swc) && swc >= 0 && swc < 1, ...", ...
"               'Pc-derived effective Swi must satisfy 0 <= Swi < 1.');", ...
"        sourceSwcBounds = sort(fluid.krPts.og(:,2));", ...
"        if swc < sourceSwcBounds(1) || swc > sourceSwcBounds(end)", ...
"            fprintf(['                Pc-derived effective Swi %.8g lies outside ', ...", ...
"                     'constituent endpoints [%.8g, %.8g]; retaining the exact ', ...", ...
"                     'upscaled endpoint for 1D matching.\n'], ...", ...
"                    swc, sourceSwcBounds(1), sourceSwcBounds(end));", ...
"        end", ...
...
[boundarySandPoro, boundarySandPerm, boundarySandSource] = ...
    dynamicBoundarySandProperties(replay, poroAll, perm, isSmear);
fault3D.Grid.boundarySandPoro = boundarySandPoro;
fault3D.Grid.boundarySandPerm = boundarySandPerm;
...
diagnostics.boundarySandSource = boundarySandSource;
